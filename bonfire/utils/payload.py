        ...
        if data_type == "text":
            for intent, prompts in intent_groups.items():
                out_file = os.path.join(
                    output_dir, f"bonfire_text_payloads_{intent}.jsonl"
                )
                intent_file_paths[intent] = out_file
                all_results.extend(BonfireTextEvasion(prompts, out_file).generate())
            ...
            for intent, prompts in intent_groups.items():
                out_file = os.path.join(
                    output_dir, f"bonfire_audio_payloads_{intent}.jsonl"
                )
                intent_file_paths[intent] = out_file
                all_results.extend(
                    BonfireAudioEvasion(prompts, out_file, format=fmt).generate()
                )
            ...
            for intent, prompts in intent_groups.items():
                out_file = os.path.join(
                    output_dir, f"bonfire_image_payloads_{intent}.jsonl"
                )
                intent_file_paths[intent] = out_file
                all_results.extend(
                ...
                merged.append(
                    {
                        ...
                        "prompt": tpl["template"].replace(
                            "[INSTRUCTIONS]", ins.get("prompt", "")
                        ),
                    }
                )
        return merged
