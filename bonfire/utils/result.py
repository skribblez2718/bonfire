...
class BonfireResult:
    ...
    def save_by_intent(
        ...
        post_process: Optional[
            Callable[[List[Dict[str, Any]], str, str, Any], None]
        ] = None,
    ) -> None:
        """
        ...
        for intent, intent_results in results_by_intent.items():
            # Sanitize intent for directory name
            safe_intent = re.sub(r"[^a-zA-Z0-9_-]", "_", str(intent))
            intent_dir = os.path.join(output_dir, safe_intent)
            os.makedirs(intent_dir, exist_ok=True)
            filename = os.path.basename(filename_format.format(intent=safe_intent))
